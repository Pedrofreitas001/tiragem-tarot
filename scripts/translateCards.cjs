#!/usr/bin/env node
/**
 * Script de Tradução em Lote para Cartas de Tarot
 *
 * Este script adiciona campos traduzidos (description_pt, love_pt, career_pt, advice_pt)
 * ao arquivo tarotData.ts
 */

const fs = require('fs');
const path = require('path');

// Dicionário de traduções para termos comuns de tarot
const tarotTerms = {
  // Elementos
  'Fire': 'Fogo',
  'Water': 'Água',
...
// Traduções completas para todas as 78 cartas
const cardTranslations = {
  ...
  "wands_5": {
    ...
    career_pt: "Competição no trabalho. Conflitos de equipe. Canaliza a energia competitiva produtivamente.",
  ...
  "pentacles_knight": {
    description_pt: "Um cavaleiro paciente em um cavalo estável contempla uma moeda em sua mão. Metodico e confiável, ele avança com determinação constante.",
...
// Função para ler o arquivo tarotData.ts
function readTarotData() {
  const filePath = path.join(__dirname, '..', 'tarotData.ts');
  return fs.readFileSync(filePath, 'utf8');
}

// Função para adicionar os campos traduzidos
function addTranslations(content) {
  let result = content;

  // Para cada carta, adicionar os campos traduzidos
  for (const [cardId, translations] of Object.entries(cardTranslations)) {
    // Encontrar o padrão para esta carta (após o campo advice)
    const pattern = new RegExp(
      `(id: "${cardId}",[\\s\\S]*?advice: "[^"]*")`,
      'g'
    );

    const replacement = `$1,\n    description_pt: "${translations.description_pt}",\n    love_pt: "${translations.love_pt}",\n    career_pt: "${translations.career_pt}",\n    advice_pt: "${translations.advice_pt}"`;
...
// Função para atualizar a interface TarotCardData
function updateInterface(content) {
  const interfacePattern = /export interface TarotCardData \{[\s\S]*?\}/;
  const newInterface = `export interface TarotCardData {
  id: string;
  name: string;
  name_pt: string;
  slug: string;
  ...
  return content.replace(interfacePattern, newInterface);
}

// Função principal
async function main() {
  console.log('🔮 Iniciando tradução das cartas de tarot...\n');

  const content = readTarotData();
  console.log('📖 Arquivo tarotData.ts lido com sucesso');

  // Atualizar interface
  let updatedContent = updateInterface(content);
  console.log('📝 Interface atualizada com novos campos');

  // Adicionar traduções
  updatedContent = addTranslations(updatedContent);
  console.log('🌎 Traduções adicionadas para todas as cartas');

  // Salvar arquivo
  const outputPath = path.join(__dirname, '..', 'tarotData.ts');
  fs.writeFileSync(outputPath, updatedContent);
  console.log('💾 Arquivo salvo com sucesso!\n');

  console.log('✅ Tradução concluída!');
  console.log(`   - ${Object.keys(cardTranslations).length} cartas traduzidas`);
  console.log('   - 4 campos por carta (description, love, career, advice)');
  console.log(`   - Total: ${Object.keys(cardTranslations).length * 4} traduções\n`);
}
